...
const EVENTS = {
  'linocut-workshop': {
    ...
    totalSeats: 12,
    bookedSeats: 9,
    images: ['carving station', 'inked block', 'finished prints', 'studio table'],
  ...
  'ceramics-open-studio': {
    ...
    totalSeats: 8,
    bookedSeats: 3,
    images: ['wheel station', 'glaze wall', 'drying shelf', 'hand-building table'],
  ...
  if (!event) {
    return (
      <section className="page">
        ...
        <Link to="/" className="btn btn-outline doodle doodle-tight" style={{ marginTop: 16 }}>
          Back home
        </Link>
      </section>
    )
  ...
  return (
    <section className="page event-page">
      <div className="event-hero">
        ...
        <h1>{event.title}</h1>
        <p style={{ fontSize: '1.05rem', color: 'var(--brown-soft)', maxWidth: '58ch' }}>
          {event.description}
        </p>
      ...
      <div className="booking-cta">
        {booked ? (
          <span className="sticky-note" style={{ fontSize: '1.2rem' }}>
            you're in — see you {activeDay.day} {activeDay.date} {activeDay.month} at{' '}
            {selectedTime}
          </span>
        ) : (
          <button
